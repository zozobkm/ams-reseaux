...
if(isset($_POST['new_last'])){
    ...
    // Construit la nouvelle IP complète
    $new_ip = $prefix.$new_last;

    echo "<pre> Modification en cours...</pre>";

    // Remplacer dans le fichier /etc/network/interfaces
    ...
    echo "<pre>  Nouvelle adresse IP  : $new_ip</pre>";
}
?>

<form method="post">
<label>Adresse réseau (fixée) :</label>
<input type="text" value="<?php echo $prefix; ?>" readonly>
<label>Dernier nombre (modifiable) :</label>
<input type="number" name="new_last" value="<?php echo $last; ?>" min="1" max="254">
